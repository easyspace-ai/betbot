...
func newClient(strict bool, opts ...Option) (*Client, error) {
	...
	proxyRoot := strings.TrimSpace(c.Config.ProxyURL)
	if proxyRoot != "" {
		if strings.TrimSpace(c.Config.CLOBWSConfig.ProxyURL) == "" {
			c.Config.CLOBWSConfig.ProxyURL = proxyRoot
		}
		if strings.TrimSpace(c.Config.RTDSConfig.ProxyURL) == "" {
			c.Config.RTDSConfig.ProxyURL = proxyRoot
		}
	}
	...
	if c.Config.HTTPClient == nil {
		if proxyRoot != "" {
			hc, err := transport.NewHTTPClientWithProxy(proxyRoot, c.Config.Timeout)
			if err != nil {
				c.InitErrors = append(c.InitErrors, &InitError{Component: "http_proxy", Err: err})
			} else {
				c.Config.HTTPClient = hc
			}
	...
	if c.builderCfg != nil && c.builderCfg.Remote != nil && c.builderCfg.Remote.HTTPClient == nil && proxyRoot != "" {
		if hc, err := transport.NewHTTPClientWithProxy(proxyRoot, 10*time.Second); err == nil {
			c.builderCfg.Remote.HTTPClient = hc
		}
	}
