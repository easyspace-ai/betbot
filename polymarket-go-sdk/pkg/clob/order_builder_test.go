...
func TestBuildMarketPriceTruncatesToTick(t *testing.T) {
	stub := newStubClient()
	stub.tickSize = 0.01
	stub.feeRate = 0

	// 0.123 with tick 0.01 must align to 0.12 (same as clob-client-v2 roundDown), not fail.
	...
	rawAmount := decimal.NewFromInt(10)
	...
	if !signable.Order.TakerAmount.Equal(expectedTaker) {
		t.Fatalf("taker amount mismatch: got %s want %s (price should truncate to 0.12)", signable.Order.TakerAmount.String(), expectedTaker.String())
	}
}

func TestBuildMarketAmountSharesTruncatesFloatNoise(t *testing.T) {
	stub := newStubClient()
	stub.tickSize = 0.01
	stub.feeRate = 0
	stub.book = clobtypes.OrderBookResponse{
		Bids: []clobtypes.PriceLevel{{Price: "0.5", Size: "1000"}},
	}

	// Float64 share counts (e.g. DB 3.13) often become 3.129999… in IEEE; must not fail max-2dp validation.
	signable, err := NewOrderBuilder(stub, mustSigner(t)).
		TokenID("123").
		Side("SELL").
		AmountShares(3.129999999999999).
		OrderType(clobtypes.OrderTypeFAK).
		Price(0.5).
		BuildMarket()
	if err != nil {
	...
	want := decimal.RequireFromString("3.12")
	if !signable.Order.MakerAmount.Equal(toFixedDecimal(want)) {
		t.Fatalf("maker amount: got %s want fixed(%s)", signable.Order.MakerAmount.String(), want.String())
	}
}

func TestBuildMarketAmountUSDCRejectsBeyondSixDecimalsAfterTruncate(t *testing.T) {
	stub := newStubClient()
	stub.tickSize = 0.01
	stub.feeRate = 0

	// Sub-micro amounts truncate to zero USDC → rejected as non-positive.
	...
	if err == nil || !strings.Contains(err.Error(), "amount must be positive") {
		t.Fatalf("expected non-positive amount error after USDC truncate, got %v", err)
	}
}
