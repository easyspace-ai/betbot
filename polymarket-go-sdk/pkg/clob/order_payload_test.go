...
func TestBuildOrderPayloadCasingAndOptions(t *testing.T) {
	...
	if s, ok := orderMap["salt"].(int64); !ok || s != 1 {
		t.Fatalf("salt should be JSON number 1 like clob-client-v2 parseInt, got %T %v", orderMap["salt"], orderMap["salt"])
	}
	if payload["deferExec"] != false {
		t.Fatalf("deferExec default: got %v", payload["deferExec"])
	}
	if payload["postOnly"] != true {
		t.Fatalf("postOnly: got %v", payload["postOnly"])
	}
}
