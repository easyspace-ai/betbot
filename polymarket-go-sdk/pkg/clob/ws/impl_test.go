...
func TestNormalizeWSURLs_Empty(t *testing.T) {
	...
	if market != ProdMarketWSSURL {
		t.Fatalf("expected market URL, got %s", market)
	}
	if user != ProdUserWSSURL {
...
func TestSubscriptionEntry_MatchesMarket_AllMarkets(t *testing.T) {
	sub := newTestSub(nil, nil)
	sub.AllMarkets = true
	if !sub.matchesMarket("any") {
		t.Fatal("AllMarkets should match any condition id")
	}
}

func TestSubscriptionEntry_MatchesMarket_NoKeysDoesNotMatch(t *testing.T) {
	sub := newTestSub(nil, nil)
	if sub.matchesMarket("m1") {
		t.Fatal("without AllMarkets, empty market set should not match")
	}
}
...
func TestAddUserRefs(t *testing.T) {
	...
	newMarkets, send := c.addUserRefs([]string{"m1"}, ap)
	if !send || len(newMarkets) != 1 {
		t.Fatalf("expected 1 new + send, got %v send=%v", newMarkets, send)
	}
...
func TestRemoveUserRefs(t *testing.T) {
	c := newTestClient()
	ap := &AuthPayload{APIKey: "k", Secret: "s", Passphrase: "p"}
	_, _ = c.addUserRefs([]string{"m1"}, ap)
...
func TestApplySubscription_UserEmptyMarketsRequiresAuth(t *testing.T) {
	c := newTestClient()
	req := &SubscriptionRequest{Type: ChannelUser}
	err := c.applySubscription(req, OperationSubscribe)
	if err == nil || !strings.Contains(err.Error(), "API key") {
		t.Fatalf("expected API key error for user subscribe without auth, got %v", err)
	}
}
