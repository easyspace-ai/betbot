...
func (c *clientImpl) resubscribe(channel Channel) {
	...
	case ChannelUser:
		...
		c.subMu.Lock()
		hasGlobal := c.userGlobalSubCount > 0
		c.subMu.Unlock()
		if len(markets) == 0 && !hasGlobal {
			return
		}
...
func (c *clientImpl) cleanupSubscriptions() {
	...
	hasGlobal := c.userGlobalSubCount > 0
	c.subMu.Unlock()
	if c.getConn(ChannelUser) != nil && (len(markets) > 0 || hasGlobal) {
		if auth == nil {
			auth = c.authPayload()
		}
		if auth != nil {
...
func (c *clientImpl) ConnectionStateStream(ctx context.Context) (*Stream[ConnectionStateEvent], error) {
		entry := newSubscriptionEntry[ConnectionStateEvent](c, ChannelMarket, ConnectionStateEventType, nil, nil, false)
	c.stateMu.Lock()
