...
const (
	// ProdBaseURL is the CLOB WebSocket host with no path (what Polymarket root Config.BaseURLs.CLOBWS uses by default).
	ProdBaseURL = "wss://ws-subscriptions-clob.polymarket.com"
	// ProdMarketWSSURL is the default public market channel (order book, prices, etc.).
	// Same as polymarket-websocket-client DEFAULT_CLOB_WSS_URL.
	ProdMarketWSSURL = ProdBaseURL + "/ws/market"
	// ProdUserWSSURL is the default authenticated user channel (orders, trades).
	// Same as polymarket-websocket-client DEFAULT_CLOB_USER_WSS_URL.
	ProdUserWSSURL = ProdBaseURL + "/ws/user"
...
type clientImpl struct {
	...
	// userGlobalSubCount is the number of user streams subscribed with markets==nil (all condition IDs).
	userGlobalSubCount int
...
func NewClientWithConfig(url string, signer auth.Signer, apiKey *auth.APIKey, cfg ClientConfig) (Client, error) {
	dialer, err := transport.WebSocketDialer(cfg.ProxyURL)
	if err != nil {
		return nil, err
	}
	c := newClientImpl(url, signer, apiKey, cfg, dialer)
...
func newClientImpl(url string, signer auth.Signer, apiKey *auth.APIKey, cfg ClientConfig, dialer *websocket.Dialer) *clientImpl {
	...
	c := &clientImpl{
		...
		wsDialer: dialer,
		wsProxy:  strings.TrimSpace(cfg.ProxyURL),
	}
...
func (c *clientImpl) Clone() Client {
	...
	clone := newClientImpl(c.baseURL, c.signer, c.apiKey, cfg, dialer)
...
func normalizeWSURLs(raw string) (string, string, string) {
	...
	default:
		if trimmed == ProdBaseURL {
			return ProdMarketWSSURL, ProdUserWSSURL, ProdBaseURL
		}
