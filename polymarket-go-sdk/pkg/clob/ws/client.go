...
type Client interface {
	...
	// Pass an empty slice (or nil) to receive updates for every market (condition id), matching official WS clients.
	// To filter, pass non-empty condition ids; do not mix with an all-markets subscription on the same client.
	...
	// Requires an API key to be configured on the client.
	// Pass an empty slice (or nil) for all markets; non-empty filters by condition id (same constraints as SubscribeUserOrders).
