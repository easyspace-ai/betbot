...
type (
	...
	BalanceAllowanceRequest struct {
		...
		TokenID string `json:"token_id,omitempty"`
		// SignatureType is the user signature type (0=EOA, 1=Proxy, 2=Gnosis Safe, 3=POLY1271).
		SignatureType *int `json:"signature_type,omitempty"`
		// Funder is the on-chain address that holds collateral for proxy/Safe/POLY1271 flows (Polymarket UI “proxy wallet”).
		// Sent as the "funder" query parameter on /balance-allowance; pair with the signature_type your account uses on the site (often 3).
	...
	BalanceAllowanceUpdateRequest struct {
		...
		// SignatureType is the user signature type (0=EOA, 1=Proxy, 2=Gnosis Safe, 3=POLY1271).
		SignatureType *int `json:"signature_type,omitempty"`
		// Funder is the collateral-holding address for proxy/Safe/POLY1271 flows (see BalanceAllowanceRequest.Funder).
		Funder string `json:"funder,omitempty"`
...
type (
	...
	Order struct {
		...
		// NegRisk selects the NegRisk CTF Exchange V2 verifying contract for EIP-712 (not sent on wire).
		NegRisk bool `json:"-"`
	}
