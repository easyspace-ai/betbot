...
func TestAccountMethods(t *testing.T) {
	...
	t.Run("BalanceAllowanceFunderQuery", func(t *testing.T) {
		funder := "0x6579c2eef112f5187bfa322e40b24b6317a43bec"
		doer := &staticDoer{
			responses: map[string]string{
				"/balance-allowance?asset_type=COLLATERAL&funder=" + funder + "&signature_type=1": `{"balance":"123","allowances":{}}`,
			},
		}
		client := &clientImpl{httpClient: transport.NewClient(doer, "http://example")}
		sig := 1
		...
		if err != nil || resp.Balance != "123" {
			t.Errorf("BalanceAllowance funder query failed: %v", err)
		}
	})
