...
// CTF Exchange V2 verifying contracts (Polygon + Amoy); matches @polymarket/clob-client-v2 config.
const (
	ctfExchangeV2Address        = "0xE111180000d2663C0091e4f400237545B87B996B"
	ctfExchangeV2NegRiskAddress = "0xe2222d279d744050d28e00520010520000310F59"
)

func ctfExchangeV2VerifyingContract(negRisk bool) string {
	if negRisk {
		return ctfExchangeV2NegRiskAddress
	}
...
func signOrderWithCreds(signer auth.Signer, apiKey *auth.APIKey, order *clobtypes.Order, sigType *auth.SignatureType, funder *types.Address, saltGen SaltGenerator) (*clobtypes.SignedOrder, error) {
	...
	domain := &apitypes.TypedDataDomain{
		...
		VerifyingContract: ctfExchangeV2VerifyingContract(order.NegRisk),
	}
	...
	var orderSigner common.Address
	switch {
	case sigTypeVal == int(auth.SignaturePoly1271):
		orderSigner = common.Address(order.Maker)
	default:
		orderSigner = common.Address(order.Signer)
		if orderSigner == (common.Address{}) {
			orderSigner = signer.Address()
		}
	}
	order.Signer = types.Address(orderSigner)
	...
	orderTypes := typesDef["Order"]
	var sig []byte
	var signErr error
	if sigTypeVal == int(auth.SignaturePoly1271) {
		exchangeAddr := common.HexToAddress(ctfExchangeV2VerifyingContract(order.NegRisk))
		sig, signErr = signExchangeV2Poly1271(signer, domain, orderTypes, order, message, sideInt, sigTypeVal, timestamp, exchangeAddr)
	} else {
		sig, signErr = signer.SignTypedData(domain, typesDef, message, "Order")
	}
	if signErr != nil {
