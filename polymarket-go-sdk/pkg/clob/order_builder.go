...
func (b *OrderBuilder) AmountUSDC(amount float64) *OrderBuilder {
	b.amount = &marketAmount{
		kind:  amountUSDC,
		value: decimal.NewFromFloat(amount).Truncate(usdcDecimals),
	}
	return b
}
...
func (b *OrderBuilder) AmountShares(amount float64) *OrderBuilder {
	b.amount = &marketAmount{
		kind:  amountShares,
		value: decimal.NewFromFloat(amount).Truncate(lotSizeScale),
	}
	return b
}
...
func (b *OrderBuilder) BuildMarketWithContext(ctx context.Context) (*clobtypes.SignableOrder, error) {
	...
	if b.price.Sign() > 0 {
		// Align to tick (same idea as clob-client-v2 roundDown(price, tick)); callers often pass float odds.
		price = b.price.Truncate(tickScale)
	} else {
		var err error
	...
	negRisk := b.resolveNegRisk(ctx, b.tokenID)
	order := &clobtypes.Order{
		Salt:          types.U256{Int: salt},
		Signer:        types.Address(orderSigner),
		Maker:         maker,
...
func (b *OrderBuilder) buildLimit(ctx context.Context) (*clobtypes.Order, error) {
	...
	price := b.price.Truncate(tickScale)
	if !price.IsPositive() {
		return nil, fmt.Errorf("price must be positive for tick size %s", tickSize.String())
	}
	one := decimal.NewFromInt(1)
	if price.LessThan(tickSize) || price.GreaterThan(one.Sub(tickSize)) {
		return nil, fmt.Errorf("price %s is out of bounds for tick size %s", price.String(), tickSize.String())
