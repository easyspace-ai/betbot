...
// ClobAuthL1Message is the fixed EIP-712 message body for CLOB L1 authentication
// (create / derive API key). Matches Polymarket clob-client / clob-client-v2.
const ClobAuthL1Message = "This message attests that I control the given wallet"

// ClobAuthTypedDataDomain returns the EIP-712 domain for CLOB L1 auth.
// Domain uses name, version, and chainId only (no verifyingContract), per official clients.
func ClobAuthTypedDataDomain(chainID *big.Int) apitypes.TypedDataDomain {
	if chainID == nil {
	...
	return apitypes.TypedDataDomain{
		Name:    "ClobAuthDomain",
		Version: "1",
		ChainId: (*math.HexOrDecimal256)(cid),
	}
}

// ClobAuthDomain is the default EIP-712 domain for Polygon mainnet (backward compatibility).
