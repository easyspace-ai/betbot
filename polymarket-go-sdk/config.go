...
type Config struct {
	...
	// ProxyURL, when non-empty, is applied to the default HTTP client (if HTTPClient is nil)
	// and propagated to CLOB and RTDS WebSocket dialers unless those configs already set
	// their own ProxyURL. Example: http://127.0.0.1:15236
	ProxyURL      string
	UserAgent     string
...
func DefaultConfig() Config {
	return Config{
		...
		ProxyURL:      transport.ProxyURLFromEnv(),
		UseServerTime: false,
